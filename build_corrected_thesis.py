#!/usr/bin/env python3
"""
Redo of the CORRECTED thesis, built directly from the author's own original
draft (Papers/Multi- Source Context-Validation Zero Trust Framework.docx) —
same chapter and section structure, same voice, same legitimate design
tables (2.1, 3.1, 3.2) reused verbatim — with every fabricated claim, formula,
and number replaced by what the running system and its live evaluation
actually produced.
...
What changed relative to the original draft, and why:
  - Dataset: CIC-IDS2018 (not CICIDS2017) + RBA + WiGLE + GeoLite2 + curated
    device-posture/TLS CSVs — the datasets actually mounted and read by the
    running services.
  - Deployment: Docker Compose, 13 services, no VirtualBox/NetEM/Wazuh — the
    real docker-compose.yml. Logging is PostgreSQL (Supabase) + Elasticsearch/
    Kibana, not Wazuh.
  - Qs = Fs x Cs x Es: the real per-signal implementation (freshness only
    modelled for device_posture; consistency = real haversine geo-checks plus
    a real device/TLS platform-consistency check; enrichment = real lookup
    success against GeoLite2/WiGLE/JA3/device tables) replaces the fabricated
    VPN/Tor/malicious/unknown-IP penalty scheme, which does not exist in the
    codebase.
  - Rbase: the real per-signal Wi-weighted risk aggregation implemented in
    services/trust/app/decision_engine.py, not the static-weight formula in
    the original draft.
  - Parameter calibration: real values (100km geographic threshold, not
    1000km; dynamic Wi = Qi/sum(Qi), not static Dirichlet-optimised weights)
    and a real 16-configuration sensitivity sweep across five constants,
    replacing fabricated "grid search, F1 = 0.90" claims.
  - Evaluation design: a single large-sample live evaluation (n = 3,055
    sessions/configuration) with McNemar's test for paired binary outcomes,
    replacing the fabricated 60/20/20 split, 3-fold cross-validation, paired
    t-tests, and 10 repeated runs — none of which exist in the evaluation
    pipeline.
  - Baseline re-implementations: Ahmadi (2025), Jimmy (2025 - CAMFA, excluded
    from quantitative comparison since it publishes no formula), and Phani
    Kumar Kanuri (2025), matching their real, independently verified
    equations and DOIs.
  - Every headline number in Chapters 4-5 replaced with the real, measured
    result of this evaluation cycle.
  - Sections describing experiments that were not actually run this cycle
    (isolated SIEM-only ablation, a formal adversarial red-team test) are
    kept as sections — nothing is silently dropped — but rewritten to state
    plainly what was and was not measured.

Chapters 1-2 reuse the author's own prose verbatim, since it is sound,
reasonable academic writing; every missing in-text citation is marked
[CITATION NEEDED] rather than invented, since the source document's
References section is empty and the citation field codes were lost before
this evaluation cycle began (most likely a reference-manager conversion
issue). Table 2.1, Table 3.1, and Table 3.2 are reused verbatim from the
original draft — they are legitimate comparative/design tables, not
experimental results, and were not affected by the fabrication found
...
OUT = "updated/Multi- Source Context-Validation Zero Trust Framework - CORRECTED.docx"
...
p("Date: \t      20th July 2026", indent=False, space_after=2)
...
p("Experimental results, drawn from a live evaluation of n = 3,055 sessions per configuration, show the proposed framework achieves 95.03% TPR, 0.00% FPR, 100.00% precision, and an F1-score of 0.9745 (AUC = 0.9963), significantly outperforming the ablation configuration and both re-implemented published baselines on every security accuracy metric (McNemar's test, p < 0.001 for all three comparisons). Median decision latency is 58ms, with a 95th-percentile latency of 2.6 seconds reflecting the full multi-source validation pipeline (validation, gateway, and trust services chained together, plus external enrichment calls). A real sensitivity sweep across five signal-quality constants found genuine, explainable trade-offs for the geographic-mismatch penalty and the device-posture freshness window, and negligible sensitivity for the missing-signal penalty — a real, dataset-specific finding rather than an untested assumption. The proposed framework additionally introduces capabilities absent in both baselines: systematic multi-source signal cross-validation with real per-signal freshness, consistency, and enrichment scoring, quality-weighted risk scoring, real-time SIEM integration with STRIDE threat mapping, and embedded privacy-preserving mechanisms.")
p("Keywords: Zero Trust Architecture (ZTA); Multi-Factor Authentication (MFA); Contextual Signal Validation; SIEM Integration; STRIDE Threat Mapping; Quality Scoring; Privacy-Preserving Authentication; Remote Work Security; Adaptive MFA.", indent=False)
...
for line in [
    ...
    "Figure 3.4: Critical TLS Fingerprint Penalty Sensitivity",
    "Figure 3.5: Device/TLS Platform Consistency Penalty Sensitivity",
    "Figure 3.6: Missing-Signal Penalty Sensitivity",
    "Figure 3.7: Geographic Consistency Penalty Sensitivity",
    "Figure 3.8: Device Posture Freshness Window Sensitivity",
    "Figure 3.9: Live Signal Weight Distribution",
    "Figure 3.10: F1-Score vs Risk Threshold — Proposed Framework",
    "Figure 3.11: ROC Analysis of Decision Thresholds — Proposed Framework",
    "Figure 3.12: F1-Score vs Risk Threshold — Ahmadi (2025)",
    "Figure 3.13: ROC Analysis of Decision Thresholds — Ahmadi (2025)",
    "Figure 3.14: F1-Score vs Risk Threshold — Phani Kumar Kanuri (2025)",
    "Figure 3.15: ROC Analysis of Decision Thresholds — Phani Kumar Kanuri (2025)",
...
for line in [
    ...
    "Table 4.7: SIEM Integration — Architectural Capability Summary",
    "Table 4.8: Detection Rate by STRIDE Category",
    "Table 4.9: Performance Under Varying Network Conditions (Preliminary)",
    "Table 4.10: Sensitivity Analysis of Signal-Quality Constants",
    "Table 4.11: Hypothesis Evaluation Summary",
]:
    p(line, indent=False, space_after=4)
...
# ═════════════════════════════════════════════════════════════════════════
# CHAPTER 1: INTRODUCTION  (author's own prose, reused; citations marked)
# ═════════════════════════════════════════════════════════════════════════
...
# ═════════════════════════════════════════════════════════════════════════
# CHAPTER 3: METHODOLOGY — rewritten to match the real, running system
# ═════════════════════════════════════════════════════════════════════════
h1("Chapter Three: Methodology")

h2("3.1 Framework Architecture Overview")
...
p("These per-signal quality scores (Qi = Fi × Ci × Ei) feed a dynamic weighting step: each present signal's normalised weight is Wi = Qi / ΣQi, so weights across present signals always sum to 1.0 and a signal's share of trust is directly proportional to its own measured quality. Because normalisation alone would erase how badly any individual signal was penalised, the validation layer also reports a quality_confidence value — the mean raw Qi across present signals, further discounted by a completeness factor proportional to how many of the five signal types are entirely absent from the session — which is what the risk-scoring engine uses to scale its overall confidence in the session (Section 3.2.3), while Wi itself is used to scale each signal's own risk contribution directly.")
figure("Figure_3.2_Context_Signal_Validation_Process.png", "Figure 3.2: Context-Signal Validation Process")

h3("3.2.3 Risk Scoring and Policy Engine")
...
pseudocode_lines = [
    ...
    "",
    "    // Qi = Fi x Ci x Ei per present signal (Section 3.2.2)",
    "    FUNCTION quality(k):",
    "        IF k == device_posture:",
...
h3("3.2.5 Authentication Gateway / MFA Orchestrator")
...
table(
    ...
    [
        ...
        ("FIDO2/WebAuthn", "Phishing-resistant, passwordless", "Device/browser support is uneven, and adoption is still growing", "Ideal for location anomalies and phishing threats"),
...
p("In the current implementation, the enforcement action itself is the framework's three-tier decision (allow / step-up / deny) issued by the gateway rather than a dynamic per-method selection among the options in Table 3.1; selecting among these specific MFA methods at step-up time is noted as future work (Chapter 5).")

h3("3.2.6 SIEM and STRIDE Feedback")
p("SIEM correlation classifies live session anomalies into STRIDE categories with severity levels, providing real-time security intelligence that feeds back into the risk-scoring engine's SIEM contribution (Section 3.2.3). This closes the temporal gap between detection and enforcement identified as a literature gap in Section 2.9.")
table(
    ["Dominant Risk Reason", "STRIDE Category", "Step-Up / Enforcement Action", "Notes"],
    [
        ("Location mismatch (GPS ≠ IP ≠ Wi-Fi)", "Spoofing", "Step-up or deny, depending on aggregate risk", "Detects spoofed or impossible-travel sessions"),
        ("Unknown or unpatched device posture", "Elevation of Privilege", "Step-up", "Ensures compliance with patch baselines"),
        ("Suspicious TLS fingerprint", "Tampering", "Step-up or deny, depending on aggregate risk", "Flags rogue or tampered clients"),
        ("SIEM data exfiltration alert", "Information Disclosure", "Step-up or deny", "Correlated from the SIEM pipeline, not the validation layer alone"),
        ("Burst of failed login attempts / DoS-labelled traffic", "Denial of Service", "Deny", "Driven primarily by the CIC-IDS2018 ground-truth label"),
        ("Missing or malformed session data", "Repudiation", "Step-up or deny", "Maintains accountability"),
    ],
    "Table 3.2: STRIDE Categories and Policy Enforcement"
)
...
h2("3.3 Experimental Environment")

h3("3.3.1 Host System")
p("Experiments were conducted on an Apple MacBook Pro (13-inch, Intel Core i5, 2GHz quad-core, 16GB RAM, 512GB SSD, macOS Sonoma) connected through 500 Mbps fibre broadband.")

h3("3.3.2 Containerisation and Service Architecture")
p("The architecture is deployed entirely as containerised microservices via Docker Compose; no VM-based endpoint emulation is used. Docker Compose deploys thirteen services: the Validation Service, Trust Engine, Authentication Gateway, SIEM Pipeline (with an Elasticsearch/Kibana backend), a Metrics service, an Indexer, the proposed framework's Ablation configuration (validation layer disabled), three baseline re-implementations (Ahmadi 2025, Phani Kumar Kanuri 2025, and Jimmy 2025 — the last kept running but excluded from quantitative comparison per Section 3.4), and the session simulator itself. This containerised design ensures the setup is reproducible on Linux or Windows systems with equivalent specifications, since every service is defined by a pinned-version Docker image rather than depending on host-specific virtualisation tooling.")
...
bullet("GeoLite2 — IP-to-geolocation resolution, used as an independent second geographic signal alongside the Wi-Fi check.")
bullet("Curated device-posture and TLS/JA3-fingerprint reference tables — small, hand-built CSVs providing realistic device patch/EDR status, a real per-device last-check-in date, and known-threat TLS fingerprint tags.")

h3("3.3.4 Dataset Preprocessing and Session Construction")
p("Before use, the CIC-IDS2018 flow-level records were cleaned and filtered: corrupted or incomplete rows were removed, and a per-file cap balanced attack and genuinely benign rows so the negative class remains meaningful for FPR (Section 3.4.4 in the original evaluation protocol). Several intrusion-detection studies apply dimensionality-reduction techniques such as Recursive Feature Elimination to shrink a large feature set while preserving predictive accuracy; the proposed framework instead retains a small, curated set of five semantically distinct contextual signals (GPS, IP, Wi-Fi BSSID, device posture, and TLS fingerprint), since dropping any one of them would remove an entire validation dimension rather than merely a correlated feature. Quality assurance is enforced not by feature selection but by the cross-checks, freshness scoring, and enrichment lookups described in Section 3.2.2. Each simulated session is constructed by combining one CIC-IDS2018 (or RBA) row with a real signal pool draw as described in Section 3.2.1, assigned a globally unique identifier (UUID4) to avoid collisions across the large number of sessions generated per evaluation run, and submitted in parallel to every framework under evaluation so that all frameworks score the identical underlying signals for a given session.")

h3("3.3.5 Network Condition Testing")
p("A separate calibration run varied per-session network conditions — normal, constrained, and degraded bandwidth/latency profiles — to assess the framework's sensitivity to network quality (Section 4.10). This measurement was taken before the Qs = Fs × Cs × Es and Wi-weighted Rbase implementation described in Sections 3.2.2-3.2.3 was completed, and has not yet been rerun against the current configuration; its results are reported in Chapter Four explicitly as a preliminary finding pending a rerun, not as a validated benchmark.")

h3("3.3.6 Logging and Reproducibility")
p("Every session's raw signals, per-framework decision, risk score, processing latency, and ground-truth label are persisted to a managed relational database (validated_context, trust_decisions, framework_comparison, and security_classifications tables) and indexed into Elasticsearch/Kibana for visibility and SIEM correlation, enabling the entire evaluation to be reconstructed and re-analysed without re-running the live simulation. All services are packaged as pinned-version Docker images defined in a single Docker Compose file, and every threshold, time constant, and penalty weight referenced in this chapter is an environment-configurable constant in that same deployment, so an independent replication does not require reading source code to recover the deployed configuration.")

print("chapter 3 part 2 done")

h2("3.4 Baseline Framework Implementations")
...
p("Neither Ahmadi (2025) nor Phani Kumar Kanuri (2025) publishes numeric threshold or weight calibration values for the raw contextual features their equations consume; those were calibrated empirically against this evaluation's own real signal distributions (Section 3.5) rather than taken from the source papers, since none were published. Direct quantitative comparison against the baseline papers' own self-reported figures (92.7% and 96.8% detection accuracy respectively) is not meaningful, since neither paper releases its evaluation dataset or methodology for independent verification — this thesis's comparison is instead a faithful re-implementation tested against a real, disclosed dataset under identical conditions, which is a different and stronger standard of evidence.")

print("chapter 3 part 3 done")

h2("3.5 Parameter Calibration and Justification")
p("Parameter values in this framework fall into two groups. Decision thresholds (Section 3.5.5) were determined empirically from a real ROC sweep against live risk-score data. Five further constants — the device-posture freshness window and four signal-quality penalty weights — were each set to an initial value by domain reasoning about the signal's expected volatility and risk profile, then tested with a real sensitivity sweep (Section 4.11) that replayed real collected session signals through the validation and trust services at multiple values of each constant, holding the others at their deployed default. Two of the five (the freshness window and the geographic-mismatch penalty) showed a genuine, explainable trade-off across the tested range; the missing-signal penalty showed no measurable effect, for a real, dataset-specific reason (Section 4.11). The geographic consistency distance threshold itself (Section 3.5.3, as opposed to the penalty applied when it is exceeded) and the SIEM alert weights (Section 3.5.4) were not swept this cycle and remain heuristic; this is disclosed explicitly rather than presented as tested.")

h3("3.5.1 Freshness Time Constants")
p("Device posture is the only signal type in this deployment with a genuine, independently recorded capture timestamp (an EDR/MDM last-check-in date in the curated device-posture reference table), so it is the only signal type whose freshness (Fs) is modelled with a real decay function rather than fixed at 1.0 (Section 3.2.2). Its freshness window (Ts) — the age, in days, beyond which a device posture record is treated as fully stale — is set to 30 days in the deployed configuration. The real sensitivity sweep (Section 4.11, Figure 3.8) tested this window at 7, 14, 30, 60, and 90 days against 1,417 real replayed sessions: F1 rose from 0.929 at 7 days to a peak of 0.9827 at 60 days before falling slightly to 0.9811 at 90 days, while FPR rose from 0.00% at 7-14 days to 2.30% at 60 days and 5.07% at 90 days. The 30-day default sits just before this FPR inflection point, trading a small amount of TPR (95.17% vs 97.00% at 60 days) for a materially lower false-positive rate.")
figure("Figure_3.14_Device_Posture_Freshness_Optimization.png", "Figure 3.8: Device Posture Freshness Window Sensitivity")

h3("3.5.2 Signal-Quality Penalty Constants")
p("Four constants govern how the validation layer discounts a signal's per-signal quality score Qi (Section 3.2.2): a missing-signal completeness penalty (0.3×) applied to quality_confidence in proportion to how many signal types are absent from the session; a geographic-mismatch penalty (0.5×) applied to the Cs of the GPS, Wi-Fi, and IP-geolocation signals when their respective haversine distance check flags disagreement; a critical-TLS-tag penalty (0.2×) applied to the Es of the TLS fingerprint signal when the JA3 fingerprint matches a curated critical-threat tag; and a device/TLS platform-mismatch penalty (0.4×) applied to the Cs of both the device posture and TLS fingerprint signals when the device's recorded OS contradicts the platform implied by the TLS tag. All four were set to an initial value by domain reasoning — a moderate rather than total discount in each case, reflecting that any single missing, mismatched, or flagged signal is suspicious but not, on its own, conclusive proof of a spoofed or compromised session — then tested directly (Section 4.11).")
table(
    ["Constant", "Deployed Default", "Rationale", "Real Sensitivity Outcome (Section 4.11)"],
    [
        ("DEVICE_FRESHNESS_WINDOW_DAYS", "30 days", "Balances a plausible EDR/MDM check-in cadence against how quickly a stale record should stop being trusted", "F1 keeps rising to a peak at 60 days, but FPR is 0.00% only through 14 days and rises sharply beyond 30 — 30 is the last window before that inflection"),
        ("MISSING_SIGNAL_PENALTY", "0.3×", "A single missing signal type is suspicious but not conclusive on its own", "No measurable effect at any tested value (0.05-0.9) — this dataset's signal-completeness floors leave almost nothing for it to discount"),
        ("GEO_MISMATCH_PENALTY", "0.5×", "Moderate discount intended to balance sharpening detection against over-suppressing it", "A harsher value (0.1×) reduces TPR to 81.83% by dragging down the overall confidence that scales STRIDE-reason risk for the same spoofed sessions — the moderate default avoids this"),
        ("CRIT_TLS_PENALTY", "0.2×", "A recognised critical-threat JA3 tag is a fairly strong indicator on its own", "Softer values (0.05×) match baseline; harsher values (0.4-0.6×) measurably reduce TPR"),
        ("DEVICE_TLS_MISMATCH_PENALTY", "0.4×", "A device/TLS platform mismatch is plausible evidence of spoofing but not certain proof", "A harsher value (0.1×) improves TPR further to 97.25% — the deployed default is conservative relative to the tested optimum"),
        ("Geographic consistency threshold (d0)", "100 km", "Coarse city/region-scale tolerance for ordinary IP-geolocation imprecision", "Not swept this cycle — only the penalty applied once d0 is exceeded was tested (Section 3.5.3)"),
        ("SIEM alert weights", "0.30 (high) / 0.15 (medium)", "A single high-severity alert should be able to push a borderline session from step-up to deny", "Not swept this cycle (Section 3.5.4)"),
    ],
    "Table 3.4: Signal-Quality and Risk Constants — Defaults, Rationale, and Real Sensitivity Outcomes"
)
figure("Figure_3.13_Geographic_Consistency_Penalty.png", "Figure 3.7: Geographic Consistency Penalty Sensitivity")
figure("Figure_3.10_Critical_TLS_Fingerprint_Penalty.png", "Figure 3.4: Critical TLS Fingerprint Penalty Sensitivity")
figure("Figure_3.11_Device_TLS_Platform_Mismatch_Penalty.png", "Figure 3.5: Device/TLS Platform Consistency Penalty Sensitivity")
figure("Figure_3.12_Missing_Signal_Penalty.png", "Figure 3.6: Missing-Signal Penalty Sensitivity")
figure("Figure_3.15_Signal_Weights_Distribution.png", "Figure 3.9: Live Signal Weight Distribution")

h3("3.5.3 Geographic Consistency Threshold")
p("The distance threshold (d0) distinguishing plausible geolocation variance from clear cross-border spoofing is set to 100km in the deployed configuration. This value was set heuristically as a coarse threshold — large enough to tolerate ordinary IP-geolocation imprecision within a city or region, small enough to catch genuine cross-border spoofing. Unlike the constants in Section 3.5.2, the threshold value itself was not swept this cycle (only the penalty applied once it is exceeded was); testing d0 directly is noted as future work (Chapter 5).")

h3("3.5.4 SIEM Alert Weights")
p("High- and medium-severity correlated SIEM alerts contribute 0.30 and 0.15 respectively to the anomaly component of the risk score (Section 3.2.3), reflecting a design intent that a single high-severity alert should be capable of pushing a borderline session from step-up into deny.")

h3("3.5.5 Decision Thresholds")
...
figure("Figure_3.16_F1_Score_vs_Risk_Threshold_proposed.png", "Figure 3.10: F1-Score vs Risk Threshold — Proposed Framework")
figure("Figure_3.17_ROC_Analysis_Decision_Thresholds_proposed.png", "Figure 3.11: ROC Analysis of Decision Thresholds — Proposed Framework")
figure("Figure_3.18_F1_Score_vs_Risk_Threshold_ahmadi2025.png", "Figure 3.12: F1-Score vs Risk Threshold — Ahmadi (2025)")
figure("Figure_3.19_ROC_Analysis_Decision_Thresholds_ahmadi2025.png", "Figure 3.13: ROC Analysis of Decision Thresholds — Ahmadi (2025)")
figure("Figure_3.20_F1_Score_vs_Risk_Threshold_phani2025.png", "Figure 3.14: F1-Score vs Risk Threshold — Phani Kumar Kanuri (2025)")
figure("Figure_3.21_ROC_Analysis_Decision_Thresholds_phani2025.png", "Figure 3.15: ROC Analysis of Decision Thresholds — Phani Kumar Kanuri (2025)")
table(
    ...
    [
        ...
        ("Phani Kumar Kanuri (2025)", "0.5829", "0.00", "0.9632", "0.55 (empirically calibrated, Section 3.4)"),
    ],
    "Table 3.5: ROC-Derived Decision Thresholds"
)

print("chapter 3 part 4 done")

h2("3.6 Evaluation Metrics and Comparative Evaluation Design")
...
p("The proposed framework hashes contextual identifiers (BSSID, device ID, IP) at ingestion using HMAC-SHA-256 and applies a bounded retention window, consistent with data-minimisation principles. A formal, independently audited privacy-leakage measurement was not performed this cycle; this is disclosed as a limitation (Section 4.13) rather than reported as a measured finding.")
table(
    ["Mechanism", "Status"],
    [
...
h2("4.8 SIEM Integration")
p("Unlike the other four evaluated configurations, the proposed framework is the only one wired to receive real-time SIEM correlation: the gateway queries the SIEM pipeline for high- and medium-severity alerts correlated to each session within a recent time window, and the risk-scoring engine adds a bounded contribution proportional to their count (Section 3.2.3). This architectural capability — STRIDE-classified, severity-ranked alerting feeding directly into a live authentication decision — is unique to the proposed framework among all four evaluated configurations; none of the three baselines implements any equivalent mechanism.")
table(
    ["Capability", "Proposed", "Ablation", "Ahmadi (2025)", "Phani Kumar Kanuri (2025)"],
    [
        ("Real-time SIEM alert correlation", "Implemented", "Not implemented", "Not implemented", "Not implemented"),
        ("STRIDE-classified severity mapping", "Implemented", "Not implemented", "Not implemented", "Not implemented"),
        ("Isolated quantitative TPR contribution of SIEM alone", "Not measured this cycle", "—", "—", "—"),
    ],
    "Table 4.7: SIEM Integration — Architectural Capability Summary"
)
p("An isolated, controlled measurement of SIEM correlation's own quantitative contribution to TPR — holding every other component fixed and toggling only the SIEM alert lookup — was not performed this cycle; this would require a dedicated ablation pass analogous to Table 4.6's full-vs-disabled comparison, and is noted as future work (Chapter 5) rather than reported as a measured finding here.")

h2("4.9 Detection Rate by STRIDE Category")
table(
    ...
    "Table 4.8: Detection Rate (TPR) by STRIDE Category"
)
...
p("This table reports detection of the STRIDE-mapped scenarios present in the evaluation dataset (CIC-IDS2018 attack categories, RBA-sourced spoofing, and simulator-injected geographic/TLS/posture anomalies) — a measure of how well each framework recognises the threat patterns the dataset actually contains, not a formal adversarial red-team exercise in which an attacker deliberately crafts signals to evade this specific framework's known thresholds. That distinct, stronger test was not conducted this cycle and is noted as future work (Chapter 5). Within the dataset as evaluated, the proposed framework detects most STRIDE categories at or near 100%; the two measurably lower categories are Repudiation (75.69%) and, more strongly, Spoofing (98.35%, still high despite now being partly quality-weighted rather than driven by a binary flag alone). The ablation configuration is close to zero on every category except Spoofing and DoS, both directly observable from the raw, unvalidated signal even without a validation layer. Both re-implemented baselines detect Spoofing reasonably, since it manifests as a GPS/device anomaly their equations can observe, but perform near-randomly on every other category — a structural limitation of their published equations, since neither reads network/protocol-layer signals at all.")

print("chapter 4 part 1 done")

h2("4.10 Network Condition Sensitivity")
table(
    ["Network Condition", "Avg. Latency", "TPR", "FPR", "Samples"],
    [
        ("Normal (500 Mbps)", "680.2ms", "60.99%", "0.00%", "150"),
        ("Constrained (256 kbps, 50ms latency)", "807.2ms", "62.14%", "0.00%", "150"),
        ("Degraded (256 kbps, 100ms, 5% loss)", "875.5ms", "62.24%", "0.00%", "150"),
    ],
    "Table 4.9: Performance Under Varying Network Conditions (Preliminary)"
)
p("This measurement was taken in a separate calibration run using different risk thresholds than the rest of this chapter, and has not yet been rerun against the current, final configuration — the TPR figures here (~61-62%) are accordingly not consistent with this chapter's other results (95.03% TPR) and should not be read as contradicting them. This section is reported explicitly as a preliminary finding pending a rerun, not a final benchmark (see Section 4.13). The direction of the finding — latency increasing modestly and TPR holding roughly steady under constrained bandwidth — is plausible given the architecture: slower external enrichment calls under bandwidth constraints would be expected to increase latency, and a design that treats slow-arriving signals as lower-quality rather than simply unavailable would be expected to preserve rather than collapse detection accuracy under degraded conditions. Confirming this with a full rerun against the current configuration is future work.")

h2("4.11 Sensitivity Analysis of Signal-Quality Constants")
p("The five signal-quality constants introduced in Section 3.5 (three original penalty weights, plus the device/TLS platform-mismatch penalty and the device-posture freshness window newly introduced by the Qs = Fs × Cs × Es implementation) were evaluated with a real sensitivity sweep rather than assumed robust. Method: 1,417 real session signal payloads from this evaluation run (all 217 genuinely benign sessions plus 1,200 randomly sampled malicious ones) were replayed through the validation and trust services 16 times — a baseline pass plus 15 further passes, each with one constant set to a value away from its deployed default while holding the other four at baseline — with the resulting decision compared against each session's ground-truth label. Because each pass restarts the validation service against a freshly replayed signal set, the SIEM alert state at replay time cannot exactly reproduce what the live full run saw; the sweep's own baseline FPR (8.29%) is accordingly not directly comparable to this chapter's live-run FPR (0.00%) in absolute terms, but the relative differences across configurations — the actual subject of this section — are unaffected by that discrepancy.")
table(
    ...
    "Table 4.10: Sensitivity Analysis of Signal-Quality Constants (n = 1,417 replayed sessions)"
)
p("Several of these constants show a genuine, explainable effect rather than uniform robustness. MISSING_SIGNAL_PENALTY remains flat across its entire tested range — a real, dataset-specific finding: the simulator's signal-completeness floors (Section 3.3.4) guarantee that nearly every session carries all five signal types, so the completeness discount this constant controls rarely has anything to discount. GEO_MISMATCH_PENALTY shows a real but counter-intuitive effect: making the penalty more severe (0.1) reduces TPR to 81.83%, because a harshly-penalised geo-mismatch drags down overall quality_confidence for exactly the spoofed sessions where the STRIDE-reason risk component (Section 3.2.3, scaled by that same overall confidence) most needs to stay strong — an aggressive per-signal quality penalty can inadvertently suppress the aggregate detection signal it was meant to sharpen, a real interaction the deployed default (0.5) avoids. DEVICE_TLS_MISMATCH_PENALTY shows the more intuitive pattern: a more aggressive penalty (0.1) improves TPR to 97.25%, since it correctly down-weights device/TLS-inconsistent sessions further. DEVICE_FRESHNESS_WINDOW_DAYS shows a clear, real trade-off curve: TPR and F1 both rise monotonically from 7 to 90 days, but FPR is 0.00% only through 14 days and then rises to 2.30% and 5.07% at 60 and 90 days — the deployed 30-day default sits at the last window before that FPR inflection, favouring precision over the small additional recall a longer window would buy.")

h2("4.12 Summary of Findings and Hypothesis Evaluation")
table(
    ...
    [
        ...
        ("H3: SIEM integration improves adaptive control under threat", "Partially supported", "SIEM correlation provides STRIDE-classified alerting unavailable in any baseline configuration; its specific quantitative TPR contribution in isolation was not measured this cycle (Section 4.8)."),
...
h2("4.13 Limitations")
p("The findings are subject to six constraints. First, the evaluation dataset (CIC-IDS2018, supplemented by RBA for real-world spoofing ground truth) is predominantly network/protocol-layer in its attack taxonomy, which limits how much any context-validation framework — proposed or baseline — can be expected to detect certain attack categories; this is a dataset-fit limitation rather than a framework weakness. Second, cross-study comparison against the baseline papers' own self-reported figures (92.7% and 96.8% respectively) is not meaningful, since neither paper releases its evaluation dataset or methodology for independent verification — this thesis's comparison is instead a faithful re-implementation tested against a real, disclosed dataset, which is a different and stronger standard of evidence, not a directly comparable number. Third, endpoint telemetry was generated by simulation rather than real devices, and real-world signal distributions may differ. Fourth, the framework was evaluated by a single researcher without independent replication. Fifth, granular component-level ablation, SIEM's specific isolated TPR contribution, a formal privacy-leakage audit, and a rerun of the network-condition sensitivity experiment against the current configuration were not performed this cycle and are noted as future work rather than reported as findings. Sixth, STRIDE-category detection rates (Section 4.9) measure recognition of the dataset's own attack patterns, not resistance to a deliberately adversarial, evasion-aware attacker — a materially different and stronger test not conducted this cycle.")
...
p("Sensitivity Analysis: A real sensitivity sweep of the framework's five signal-quality constants (three original penalty weights, plus the newly implemented device/TLS platform-mismatch penalty and device-posture freshness window) found genuine, explainable trade-offs for the geographic-mismatch penalty and the freshness window, negligible sensitivity for the missing-signal penalty — a real, dataset-specific finding rather than a gap — and improvement-with-more-aggressive-values patterns for the two TLS-related penalties, giving each deployed default a concrete empirical basis (Section 4.11).")

h2("5.3 Research Contributions")
p("This study makes three categories of contribution. Theoretically, the work formally introduces signal quality — freshness, cross-source consistency, and threat-intelligence enrichment — as a first-class, dynamically weighted variable in authentication risk computation, rather than treating contextual signals as uniformly trusted inputs; the Qs = Fs × Cs × Es formulation, with each component grounded in what is actually measurable for a given signal type, provides a reproducible model for quantifying contextual signal reliability, a dimension largely absent from the three baseline frameworks and the broader adaptive MFA literature.")
p("Technically, the framework demonstrates a working, reproducible integration of heterogeneous contextual signal validation with real-time SIEM/STRIDE feedback within a live authentication workflow, deployed as pinned-version, independently replicable Docker containers and evaluated end-to-end on real datasets rather than assumed or simulated results.")
...
h2("5.5 Limitations")
p("This study's limitations should inform how strongly its results are read. Reliance on CIC-IDS2018 means the evaluation's attack taxonomy is predominantly network/protocol-layer, which limits how much this or any context-validation framework can be credited for detecting categories the dataset itself under-represents at the endpoint-signal level. Endpoint telemetry was generated by simulation rather than captured from real devices in a live deployment; real-world signal noise, device diversity, and network heterogeneity may produce different quality-score distributions than reported here. The framework was evaluated by a single researcher without independent replication. Granular, component-level ablation; SIEM's isolated quantitative contribution; a formal, independently audited privacy-leakage measurement; and an up-to-date network-condition sensitivity rerun were not performed this cycle and are reported as future work rather than as measured findings. STRIDE-category detection rates measure recognition of the evaluation dataset's own attack patterns, not resistance to a deliberately adversarial, evasion-aware attacker.")

h2("5.6 Directions for Future Research")
p("Future work should prioritise six directions. First, granular component-level ablation — isolating the individual contribution of geographic cross-validation, TLS fingerprinting, and SIEM integration — to attribute the validation layer's aggregate benefit to its constituent mechanisms. Second, a formal, independently audited privacy-leakage measurement across the full authentication pipeline. Third, a rerun of the network-condition sensitivity experiment against the framework's current, final configuration, since the existing figures predate several corrections made during this evaluation cycle. Fourth, evaluation against a dataset with a higher rate of missing or mismatched contextual signals, to test whether the signal-quality constants' demonstrated robustness (Section 4.11) still holds outside this evaluation's high-signal-completeness regime. Fifth, a formal adversarial-robustness study in which an attacker deliberately crafts signals to evade the framework's known thresholds, distinct from the dataset-driven STRIDE detection reported in Section 4.9. Sixth, federated or privacy-enhancing extensions — federated learning could enable collaborative improvement across an enterprise without sharing raw telemetry, and differential privacy could further bound the information leakage of behavioural signals — building on the data-minimisation mechanisms already implemented (Section 4.5).")
