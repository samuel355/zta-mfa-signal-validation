...
from .ablation_engine import process_baseline_request, get_baseline_thesis_metrics, reset_baseline_metrics
...
def _index_baseline_to_es(decision: Dict[str, Any], signals: Dict[str, Any]):
    ...
    try:
        with httpx.Client(timeout=3, headers=headers, auth=auth) as c:
            ...
            print(f"[ABLATION] Indexed to ES: mfa-events({r1.status_code}), baseline-decisions({r2.status_code})")
...
@api.get("/health")
def health():
    return {"status": "ok", "service": "ablation-mfa"}

@api.post("/decision")
def baseline_decision(request: BaselineRequest, background_tasks: BackgroundTasks):
    """Make MFA decision using baseline (traditional) logic"""

    decision = make_baseline_decision(request.signals)

    # Persistence is async — off the decision-latency critical path
    background_tasks.add_task(store_baseline_decision, decision, request.signals)

    # Add OTP for MFA step-up simulation
    if decision["enforcement"] == "MFA_REQUIRED":
        decision["otp_demo"] = totp.now()

    decision["persistence"] = {"ok": "scheduled"}
