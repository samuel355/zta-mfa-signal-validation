...
api = FastAPI(title="Gateway Service", version="0.3")
...
def index_to_es(session_id: str, enforcement: str, risk: float, decision: str, reasons: list[str] | None):
    ...
    if not es_host: 
        print("[ES_INDEX] ES_HOST not set; skipping"); return
    doc = {
...
def dbcheck():
    ...
    try:
        with eng.connect() as c: c.execute(text("select 1")); return {"ok": True}
...
def dnscheck():
    ...
    try:
        ...
        if host is None: return {"ok": False, "error": "No hostname found in DB_DSN"}
...
def decision(payload: ValidateAndDecide):
    ...
    if eng is not None:
        try:
            params = {
                ...
                "detail": json.dumps({"risk": risk, "decision": decision, "enforcement": enforcement, "reasons": reasons, **detail}),
            }
    ...
    if "otp_demo" in detail: resp["otp_demo"] = detail["otp_demo"]
    return resp